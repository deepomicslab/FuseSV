...
sub find_all_connection{
	...
	for my $str_Aref (@$HapTypeStr_Aref){
		...
		for my $conn_idx_pair (keys %$Connect_Href){
			...
			$conn_dist += $SegmentLen_Href->{$str_Aref->[$_]} for ( $lf_idx .. $rt_idx );
			# compare
			my $max_nearest_dist = max($upstream_dist, $downstream_dist);
			if($max_nearest_dist == 0){
				$max_nearest_dist += $SegmentLen_Href->{$str_Aref->[$_]} for ( 0 .. $SegAmount-1 );
				$max_nearest_dist -= $conn_dist;
			}
			# get ceiling-distance
			my $ceil_dist = $conn_dist + $max_nearest_dist;
			# record
