package FuseSV::Virus_Integ::DrawSegCN::DrawSegCN;
...
my @functoion_list = qw/
                        ...
                        draw_segmemt_CN
                        prepare
                        load_prioCNofControl
                        calc_GMofTN
                        prep_regionInfo
                        load_juncInfo
                        load_vsegInfo
                        map_junction_to_segments
                        get_SegmID_by_PosLoc
                        is_in_draw_region
                        deal_with_showCNopt
...
sub Load_moduleVar_to_pubVarPool{
    $V_Href->{ $_->[0] } = $_->[1] for
        ...
        (
            ...
            [ dHG_CNlineWidth => 1 ],
            [ dHG_haploDepthInPureTcell => undef ],
            # draw options
...
sub Get_Cmd_Options{
    ...
    $V_Href->{dHG_bam}->{case} = BioFuse::BioInfo::Objects::SeqData::Bam_OB->new( tissue => 'case', filepath => undef );
    $V_Href->{dHG_bam}->{ctrl} = BioFuse::BioInfo::Objects::SeqData::Bam_OB->new( tissue => 'ctrl', filepath => undef );
...
sub calculate_CopyNumber{
    ...
    if( defined $V_Href->{dHG_CNbaseOnReg}->{depth2cn_ratio} ){
        $V_Href->{dHG_haploDepthInPureTcell} = sprintf "%.2f", 1 / $V_Href->{dHG_CNbaseOnReg}->{depth2cn_ratio};
        stout_and_sterr "[INFO]\tThe depth of haplotype in PURE case sample (tumor) is $V_Href->{dHG_haploDepthInPureTcell} X.\n";
    }
