package FuseSV::Virus_Type::CandReads;
...
$MODULE_NAME = 'FuseSV::Virus_Type::CandReads';
#----- version --------
$VERSION = "0.17";
$DATE = '2019-05-26';
...
sub CandReads{
    ...
    if($V_Href->{stepToStart} > 1){
        ...
        reset_folder(folder => $V_Href->{CandReadsFolder});
    }
...
sub setReadsGroup{
    ...
    for my $RGid (sort keys %{$V_Href->{RG}}){
        ...
        my $Cand_smartFqGzPath = GetPath(filekey => 'Cand_smartFqGz', CandFq_RG_ID_NO => $RG_NO);
        my $Cand_smartFqGz = BioFuse::BioInfo::Objects::SeqData::FastQ_OB->new(filepath => $Cand_smartFqGzPath, tag => 'Cand_smartFqGz');
        $rgOB->addAffix(Cand_smartFqGz => $Cand_smartFqGz);
    }
}
...
sub getCandReadsFromBam{
    for my $bam (@{$V_Href->{SampleOB}->bam}){
        ...
        # start rgOB Cand_nSortBam and Cand_smartFqGz
...
sub PEobToCand{
    ...
    for my $pe_OB (@$pe_OB_poolAf){
        ...
        if(  (   $V_Href->{discard_DupReads} # if discard dup
          ...
          || (   $r1_OB->rlen < $V_Href->{min_Rlen} # min rlen
              || $r2_OB->rlen < $V_Href->{min_Rlen}
             )
        ){
            next;
        }
        ...
        if(  (   $r1_OB->mapQ < $V_Href->{min_mapQ} # low mapQ
          ...
          || (   !exists $V_Href->{hostRefSeg}->{$r1_OB->mseg}
            ...
            for my $rOB ($r1_OB, $r2_OB){
                ...
                $rOB->add_BarcToPid(barcSAMtag => $V_Href->{barcSAMtag}, barcPidPrf => $V_Href->{barcPidPrf}) if defined $V_Href->{barcSAMtag};
                $rgOB->affix(key=>'Cand_smartFqGz')->write(content => $rOB->printFQ."\n");
            }
        }
    }
}
...
#--- write reads group report ---
sub makeRGreport{
    open (RGRP,Try_GZ_Write($V_Href->{Cand_RgInfoReport})) || die "fail $V_Href->{Cand_RgInfoReport}: $!\n";
    # theme
    ...
    for my $rgOB (sort {$a->RG_NO<=>$b->RG_NO} values %{$V_Href->{RG}}){
        ...
        print RGRP join ("\t",  $rgOB->RG_NO,
                                ...
                                basename($rgOB->affix(key=>'Cand_smartFqGz')->filepath)
                        )."\n";
...
#--- load RG info report ---
sub loadRGreport{
    open (RGRP,Try_GZ_Read($V_Href->{Cand_RgInfoReport})) || die "fail read $V_Href->{Cand_RgInfoReport}: $!\n";
    # theme
    ...
    while(<RGRP>){
        ...
        $rgOB->addAffix(Cand_nSortBam => $Cand_nSortBam);
        # candidate reads: smart fastq.gz
        my $Cand_smartFqGzPath = GetPath(filekey => 'Cand_smartFqGz', CandFq_RG_ID_NO => $k2v{rg_no});
        my $Cand_smartFqGz = BioFuse::BioInfo::Objects::SeqData::FastQ_OB->new(filepath => $Cand_smartFqGzPath, tag => 'Cand_smartFqGz');
        $rgOB->addAffix(Cand_smartFqGz => $Cand_smartFqGz);
        # max rlen
    ...
    close RGRP;
    # inform
    stout_and_sterr "[INFO]\tload RG info report.\n";
}
