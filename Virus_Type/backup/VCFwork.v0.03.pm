...
sub vcf_from_bam{
	...
	my $vcf = "$Out_pref.vcf";
	my $call_vcf_command = "$V_Href->{samtools} mpileup -t 'DP,AD,ADF,ADR,SP,INFO/AD,INFO/ADF,INFO/ADR' -A -d 1000000 -v -O -s -m 3 -L 1000000 -f $Ref_IndexByBwa -o $vcf $bam_for_call";
...
sub mpileup_from_bam{
	...
	my $call_vcf_command = "$V_Href->{samtools} mpileup -A -d 1000000 -O -s -m 3 -L 1000000 -f $Ref_IndexByBwa -o $mpileup $bam_for_call";
...
sub Is_I16_Alt_OK{
	...
	my $I16_min_mean_tailDist = $V_Href->{I16_min_mean_tailDist};
			...
			if(    scalar(@alt) != 0
				...
				if(!$is_indel){ # SNP, i donot want to use unless-else; once it isn't noted as INDEL, it must be one letter.
					for my $i (0 .. $#alt){
						my $alt_allel = $alt[$i];
						# record
						my $mutation_id = "snp,$alt_allel";
						# assign the SNV to current position
						my $aimPos_Href = $RefPos_Href->{$refpos};
						$aimPos_Href->{mutation}->{$mutation_id} = [ $Allel_D[$i], $Allel_D_Fw[$i], $Allel_D_Rv[$i] ];
					}
				}
				else{ # indels
					my $ref_allel = $rgOb{ref};; # this is the string around this pos
					for my $i (0 .. $#alt){
						my $alt_allel = $alt[$i];
